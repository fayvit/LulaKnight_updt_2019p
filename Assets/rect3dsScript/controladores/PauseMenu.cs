...
public class PauseMenu
{
    ...
    [SerializeField] private MenuDosMilitantes menuMilitantes = null;
    ...
    [SerializeField] private Image[] interAba = null;
    [SerializeField] private Sprite[] spriteAbas = null;
    [SerializeField] private Sprite[] spriteAbas_brancos = null;
    ...
    {
        estado = EstadoDaqui.aberturaBloqueada;
    }
    ...
    {
        if (GameController.g.Manager.Estado != EstadoDePersonagem.derrotado && estado!=EstadoDaqui.aberturaBloqueada)
        {
            Time.timeScale = 0;
    ...
    public void BtnPauseMenu()
    {
        if (estado == EstadoDaqui.emEspera)
            IniciarMenuDePause();
        else if (estado != EstadoDaqui.menuDeOpcoesAberto 
            && 
            estado != EstadoDaqui.menuSuspensoAberto 
            && 
            estado != EstadoDaqui.aberturaBloqueada)
    ...
    void DestacarAba(int qual)
    {
        for (int i = 0; i < abas.Length;i++)
        {
            if (qual == i)
            {
                ...
                abas[i].color = Color.red;
                interAba[i].sprite = spriteAbas_brancos[i];
            }
            else
            {
                abas[i].sprite = padrao;
                abas[i].color = Color.white;
                interAba[i].sprite = spriteAbas[i];
            }
        }
    }
    ...
    {
        EventAgregator.Publish(EventKey.finishAllTabPauseMenu);
        menuDePauseBasico.FinalizarHud();
        menuU.FinalizarHud();
        menuOE.FinalizarHud();
        menuI.FinalizarHud();
        mapPanel.OnExitMapaPanel();
        menuMilitantes.FinalizarHud();
    }
