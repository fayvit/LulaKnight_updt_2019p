...
#pragma warning restore 0649
    ...
    public bool NoChao
    {
        get {
            ...
            if (!estaNoChao && estavaNoChao)
            {
                estavaNoChao = false;
                retornoDonoChao = true;
                CoroutineObject.Instance.StartCoroutine(PuloFantasma());
            }
    ...
    public void Move(float move, bool jump,bool temPuloDuplo)
    {
        ...
        if (m_Grounded || m_AirControl)
        {
            ...
            if (m_Grounded && jump && !myJump.EstouPulando)
            {
                ...
                EventAgregator.Publish(new StandardSendGameEvent(m_Rigidbody2D.gameObject,EventKey.animaIniciaPulo));
            }
            ...
            if (m_Grounded && !myJump.EstouPulando)
            {
                EventAgregator.Publish(new StandardSendGameEvent(m_Rigidbody2D.gameObject, EventKey.boolDescendoPulo));
            }
        }
    }
    ...
    public void ApplyForce(Vector2 f,float tempoAplicando = 0.25f)
    {
        ...
        CoroutineObject.Instance.StartCoroutine(RetornaAplicandoForca(tempoAplicando));
    }
