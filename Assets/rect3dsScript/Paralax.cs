...
public class Paralax : MonoBehaviour {
    ...
    [SerializeField] GameObject[] paralaxObjects = default;
    [SerializeField] DadosDeCena.LimitantesDaCena limitantes = default;
	...
	void Start () {
        ...
        SetarParalaxLimits();
        myCam = FindObjectOfType<Camera2D>().transform;
	}
    ...
    public static DadosDeCena.LimitantesDaCena SetarLimits()
    {
        ...
        GameObject G = GameObject.FindWithTag("startBlock");
        ...
        G = GameObject.FindWithTag("endBlock");
        if (G != null)
        {
        ...
        for (int i = 0; i < paralaxObjects.Length; i++)
        {
            if (paralaxLimits[i] != null)
            {
                ...
                paralaxObjects[i]. transform.position = new Vector3(
                    Mathf.Lerp(xMin, xMax, (myCam.position.x - xMin) / (xMax - xMin)),
                    Mathf.Lerp(yMin, yMax, (myCam.position.y - yMin) / (yMax - yMin)),
                    0);
