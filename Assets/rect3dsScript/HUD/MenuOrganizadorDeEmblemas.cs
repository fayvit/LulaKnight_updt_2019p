...
public class MenuOrganizadorDeEmblemas
{
    ...
    void ColocaInfoTexts(Emblema E)
    {
        ...
        infoArea.text = BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.botonsInfo)[indice];

        if (E.EspacosNecessarios > 0)
        {
            custoDeEspacos.text = string.Format(BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[6],
    ...
    {
        infoTitle.text = BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[4];
        infoArea.text = BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[5];
        custoDeEspacos.text = "";
    }
    ...
    void DelayFocusPreAction(int qual,MenusDeEmblemabase oDeRetirar,MenusDeEmblemabase oDeDestaque, System.Action<int> acao)
    {
        oDeRetirar.RetirarDestaques();

        if (!estadoDeAcao)
        {
            estadoDeAcao = true;

            oDeDestaque.MudarSelecaoParaEspecifico(qual);
    ...
    {
        DelayFocusPreAction(qual, emblemasE, emblemasD, EmblemaDisponivelSelecionado_b);
    }

    void EmblemaDisponivelSelecionado_b(int qual)
    {
        if (estaNoCheckPoint)
        {
            ...
            if (!E.EstaEquipado)
            {
                if (E.EspacosNecessarios <= dj.EspacosDeEmblemas - Emblema.NumeroDeEspacosOcupados(dj.MeusEmblemas))
                {
                    ...
                    GlobalController.g.UmaMensagem.ConstroiPainelUmaMensagem(OnCheckPanel,
                        string.Format(
                        BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[0],
                        E.NomeEmLinguas
                        )
                        );
                ...
                else
                {
                    ...
                    GlobalController.g.UmaMensagem.ConstroiPainelUmaMensagem(OnCheckPanel,
                        string.Format(
                        BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[1],
            ...
            else
            {
                EventAgregator.Publish(new StandardSendGameEvent(EventKey.triedToChangeEmblemNoSuccessfull));
                GlobalController.g.UmaMensagem.ConstroiPainelUmaMensagem(OnCheckPanel,
                    BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[2]);
            }
        }
        else
        {
    ...
    void EncaixeDeEmblemaSelecionado(int qual)
    {
        ...
        DelayFocusPreAction(qual, emblemasD, emblemasE, EncaixeDeEmblemaSelecionado_b);
        
    }

   void EncaixeDeEmblemaSelecionado_b(int qual)
    {
        if (estaNoCheckPoint)
        {
            int opcaoGuardada = qual;
            ...
            else
            {
                EventAgregator.Publish(new StandardSendGameEvent(EventKey.triedToChangeEmblemNoSuccessfull));
                GlobalController.g.UmaMensagem.ConstroiPainelUmaMensagem(OnCheckPanel,
                    BancoDeTextos.RetornaListaDeTextoDoIdioma(ChaveDeTexto.frasesDeBotons)[3]);
            }
        }
        else
        {
