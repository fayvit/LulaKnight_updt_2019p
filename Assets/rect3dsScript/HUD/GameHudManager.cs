...
#pragma warning disable 0649
    ...
    [SerializeField] private GameObject lifeBarBaseUpdate;
    [SerializeField] private List<Image> lifeBarUpdatesAdd = new List<Image>();
    [SerializeField] private GameObject lifeBarBaseExtraUpdate;
    [SerializeField] private List<Image> lifeBarExtraUpdatesAdd = new List<Image>();
    [SerializeField] private GameObject magicBarBaseUpdate;
    [SerializeField] private List<Image> magicBarUpdatesAdd = new List<Image>();
...
#pragma warning restore 0649
    ...
    private void OnChangeVisibleUpdateMagicBar(IGameEvent obj)
    {
       ...
       /* for (int i = 0; i < magicBarUpdates.Length; i++)
        ...
        for (int i = magicBarUpdatesAdd.Count; i < (int)ssge.MyObject[0]; i++)
        {
            GameObject G = InstanciaLigando.Instantiate(magicBarBaseUpdate, Vector3.zero);
            G.GetComponent<RectTransform>().SetParent(magicBarBaseUpdate.transform.parent);
            G.transform.localScale = new Vector3(1, 1, 1);
            magicBarUpdatesAdd.Add(G.transform.GetChild(1).GetComponent<Image>());
        }
    }
    ...
    private void OnChangeVisibleUpdateLifeBar(IGameEvent obj)
    {
        ...
        for (int i = lifeBarUpdatesAdd.Count; i < (int)ssge.MyObject[0];i++)
        {
            GameObject G = InstanciaLigando.Instantiate(lifeBarBaseUpdate,Vector3.zero);
            G.GetComponent<RectTransform>().SetParent(lifeBarBaseUpdate.transform.parent);
            G.transform.localScale = new Vector3(1, 1, 1);
            lifeBarUpdatesAdd.Add(G.transform.GetChild(1).GetComponent<Image>());
        }
    }
    ...
    private void OnChangemagicPoints(IGameEvent obj)
    {
        ...
        PreencherBarrasDeUpdate(magicBarUpdatesAdd.ToArray(), addMagicBarAmount, maxBasicMagicBar, (int)ssge.MyObject[0], (int)ssge.MyObject[1]);
    ...
    private void OnChangeLifePoints(IGameEvent obj)
    {
        ...
        PreencherBarrasDeUpdate(lifeBarUpdatesAdd.ToArray(), addLifeBarAmount, maxBasicLifeBar, (int)ssge.MyObject[0], (int)ssge.MyObject[1]);
