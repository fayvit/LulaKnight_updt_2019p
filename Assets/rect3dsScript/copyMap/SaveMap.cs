...
//        Debug.Log(Application.dataPath + " : " + Application.persistentDataPath + " : " + Application.consoleLogPath + " : " + Application.streamingAssetsPath + " :" + Application.temporaryCachePath);
        var file = File.Open(Application.persistentDataPath +"/" +nomeDoMapa+".png" , FileMode.Create);
        var binary = new BinaryWriter(file);
        binary.Write(bytes);
        file.Close();
    }

    public static Texture2D Carregar(string nomeDoMapa = "teste")
    {
        
        Texture2D retorno = new Texture2D(2, 2);

        string pathFile = Application.persistentDataPath + "/" + nomeDoMapa + ".png";

        if (File.Exists(pathFile))
        {            
            var file = File.ReadAllBytes(pathFile);
            retorno.LoadImage(file);          
