...
public static class TradeManager
{
    public static void OnBuy(NomeMercadoria n,int quantidade = 1)
    {
        switch (n)
        {
            case NomeMercadoria.esferaDeCoalisao:
            case NomeMercadoria.chaveDeCidade:
            ...
            break;
            case NomeMercadoria.militanteDeProtesto:
            case NomeMercadoria.militanteDePerseveranca:
                new MyInvokeMethod().InvokeAoFimDoQuadro(() =>
                {
                    EventAgregator.Publish(new StandardSendGameEvent(EventKey.buyUpdateGeometry, n == NomeMercadoria.militanteDePerseveranca));
                });
            break;
