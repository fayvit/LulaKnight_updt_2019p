...
{
    [SerializeField] private Collider2D normal = null;
    [SerializeField] private Collider2D emGiro = null;
    ...
    void FinalizaGiro()
    {
        ...
        {
            if(estado == EstadoDaqui.pulinhosPreparatorios)
                _Animator.SetTrigger("retornaAoPadrao");
        }, 0.8f * TEMPO_ENTRE_PULINHO);

        normal.enabled = true;
        emGiro.enabled = false;
        ...
        switch (estado)
        {
            case EstadoDaqui.pulinhosPreparatorios:
                ...
                else if (tempoDecorrido > TEMPO_ENTRE_PULINHO && contDePulinhos >= PULINHOS_ATE_ATAQUE)
                {
                    ...
                    new MyInvokeMethod().InvokeNoTempoDeJogo(() => {
                        if (estado == EstadoDaqui.giroReto||estado==EstadoDaqui.giroAlto)
                        {
                            normal.enabled = false;
                            emGiro.enabled = true;
                            _Animator.SetTrigger("atkGiro");
                        }
                    }, TEMPO_TELEGRAFANDO_GIRO);
